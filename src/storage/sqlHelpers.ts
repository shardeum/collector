...
export function rowPlaceholders(fieldCount: number, startIndex = 1): string {
  if (pgEnabled) {
    return Array(fieldCount).fill(0)
...
export function bulkInsertSQL(
  ...
  if (pgEnabled) {
    ...
    const getParamIndex = (recordIndex: number, fieldIndex: number) => 
 ...
 */
export function whereIn(column: string, valueCount: number, startIndex = 1): { 
  sql: string, 
  endIndex: number 
} {
  if (pgEnabled) {
 ...
 * @returns Object with SQL fragment and updated end index
 */
export function between(column: string, startIndex = 1): {
  sql: string,
  endIndex: number
} {
  if (pgEnabled) {
